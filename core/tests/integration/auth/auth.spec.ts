...
describe("@auth user registration", () => {
  ...
  test("can confirm email with verification code and set new password", async ({
    ...
    const redisSessionsForUser = await container
      .make(RedisSessionStore)
      .list(user.id)

    expect(redisSessionsForUser).toHaveLength(1)

    expect(loginResponse.headers.getSetCookie()?.[0]).toMatch(
      "__Secure-session="
    )
  })
})
...
describe("@auth user login", () => {
  ...
  test("can logout, destroying currently active session", async ({
    expect,
  }) => {
    const { user } = await createUser()
...
describe("@oauth ", () => {
  ...
  test("can initiate user oauth2 register flow for github", async ({
    expect,
  }) => {
    const response = await makeRequest(
      "/auth/register/oauth2/github/authorize",
      {
        method: "GET",
      }
    )
  ...
  test(
    ...
    { retry: 3 },
    async ({ expect }) => {
      const { FakeDriver, user } = getFakeOauthProviderDriver(
        "google",
        "register"
      )
  ...
  test("user can login with github after previous registration", async ({
    expect,
  }) => {
  ...
  test("user cannot login with github if not previously registered with github", async ({
    ...
    expect(flash).toEqual({
      ...
      description:
        "Please register a new account if you haven't done so before.",
      variant: "error",
    })
