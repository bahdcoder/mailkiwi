...
import { relations } from "@kibamail/framework/mysql";
import {
  type AnyMySqlColumn,
  boolean,
  float,
...
import {
  ...
  createOauth2AccountsReferencesColumns,
  createOauth2AccountProviderContraints,
  createPasswordResetsReferencesColumns,
  createAccessTokensReferencesColumns,
  createTeamsReferencesColumns,
  createTeamMembershipsReferencesColumns
...
export const oauth2Accounts = mysqlTable(
  "oauth2Accounts", {
    ...oauth2AccountsColumns,
    ...createOauth2AccountsReferencesColumns(users)
  },
  (table) => [
    createOauth2AccountProviderContraints(table)
  ]
);

export const passwordResets = mysqlTable("passwordResets", {
  ...passwordResetsColumns,
  ...createPasswordResetsReferencesColumns(users)
});
...
export const teams = mysqlTable('teams', {
  ...teamsColumns,
  ...createTeamsReferencesColumns(users),
...
export const accessTokens = mysqlTable("accessTokens", {
  ...accessTokensColumns,
  ...createAccessTokensReferencesColumns(users, teams)
});
...
export const teamMemberships = mysqlTable("teamMemberships", {
  ...teamMembershipsColumns,
  ...createTeamMembershipsReferencesColumns(users, teams)
});
...
export const websitePages = mysqlTable(
  ...
  (table) => [unique('websiteIdPathKey').on(table.websiteId, table.path)],
)
...
export const contacts = mysqlTable(
  ...
  (table) => [unique('ContactEmailAudienceIdKey').on(table.email, table.audienceId)],
)
...
export const contactProperties = mysqlTable(
  ...
  (table) => [unique('propertyNameContactIdKey').on(table.name, table.contactId)],
)
...
export const tags = mysqlTable(
  ...
  (table) => [unique('tagNameAudienceIdKey').on(table.name, table.audienceId)],
)
...
export const tagsOnContacts = mysqlTable(
  ...
  (table) => [
    unique('tagsOnContactsTagIdContactIdUniqueKey').on(table.tagId, table.contactId),
    index('tagsOnContactsTagIdContactIdIndexKey').on(table.tagId, table.contactId),
  ],
)
...
export const broadcastGroups = mysqlTable(
  ...
  (table) => [unique('broadcastGroupNameOnTeamIdKey').on(table.teamId, table.name)],
)
...
export const messages = mysqlTable(
  ...
  (table) => [
    index('messageChannelIdIndex').on(table.channelId),
    index('messageCreatedAtIndex').on(table.createdAt),
  ],
)
...
export const senderIdentityRelations = relations(
  senderIdentities,
  ({ one }) => ({
    team: one(teams, {
