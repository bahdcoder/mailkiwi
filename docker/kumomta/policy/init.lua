...
local SINK_DATA_FILE =
  os.getenv"SINK_DATA" or "/opt/kumomta/etc/policy/responses.toml"
...
local function load_data_for_domain(domain)
  ...
  config.bounces = data.bounce[domain] or { {
    code = 550,
    msg = "boing!",
  } }
  config.defers = data.defer[domain] or { {
    code = 451,
    msg = "later!",
  } }
  return config
