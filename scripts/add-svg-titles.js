...
async function processFile(filePath) {
  ...
  // Skip if already has a title element
  if (content.includes('<title>')) {
    console.log(`Skipping ${filePath} - already has title element`)
    return
  }
  ...
  if (iconNameMatch) {
    ...
    // Try to extract from the filename if not found in the content
  ...
  // Add title element directly to the SVG
  let newContent = content.replace(
    /(<svg[^>]*>)\s*/,
    `$1\n      <title>${accessibleTitle}</title>\n      `,
  )

  // Add role and aria-label attributes to the SVG
  newContent = newContent.replace(
    /(<svg[^>]*)(>)/,
    `$1 role="img" aria-label="${accessibleTitle}"$2`,
  )
